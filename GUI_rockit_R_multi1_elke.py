...
def gui_rock(init_loc,fin_loc):
    ...
    # Rectangular collision avoidance
    ...
    for i in range(Number_C):
        ...
        ax_r[i] = ocp.control(order=1)
        ay_r[i] = ocp.control(order=1)
        b_r[i] = ocp.control(order=1)
...
if __name__ == '__main__':
    ...
    init=[[0,0,pi/2],[0.7,0,pi/2],[1.4,0,pi/2]]
    finit = [[3,3,0],[3,3.7,0],[3,4.4,0]]
