...
from GUI_rockit_R_expw import gui_rock
...
def controller(ref,inp, x, y, theta,ff):
        ...
        vxff = vxff1 * np.cos(ini[2]) - vyff1 * np.sin(ini[2])
        vyff = vxff1 * np.sin(ini[2]) + vyff1 * np.cos(ini[2])
        ...
        if ff==1:
            ...
            if vy_r>0:
              ...
              if omega>0:
                ...
                vx_r = vx_r+-0.22*vy_r+0.044*omega
                vy_r = vy_r+0.15*vx_r+0.025*omega
...
def do_control(x_in,q,waypoints,p_err):
    ...
    textfile = open("a_file_sn1c.txt", "w")
    t_eag=0
    flag=1
