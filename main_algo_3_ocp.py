...
def gui_rock(init_loc,fin_loc,a,N,M):
    ...
    for i in range(robotN):
        ...
        # Initial constraints
        ocp.subject_to(ocp.at_t0(x[i]) == Init_con[0])
        ocp.subject_to(ocp.at_t0(y[i]) == Init_con[1])
        ocp.subject_to(ocp.at_t0(theta[i]) == Init_con[2])

        # Final constraint
        ocp.subject_to(ocp.at_tf(x[i]) == Fin_con[0])
        ocp.subject_to(ocp.at_tf(y[i]) == Fin_con[1])
        ocp.subject_to(ocp.at_tf(theta[i]) == Fin_con[2])
        ...
        in_x[i] = np.linspace(Init_con[0], Fin_con[0], N)
        in_y[i] = np.linspace(Init_con[1], Fin_con[1], N)
        in_theta[i] = np.linspace(Init_con[2], Fin_con[2], N)
    ...
    waypoints=np.zeros((len(ts),3),dtype=float)
    ...
    if ani==1:
        ...
        x_d=x_d_all[:,0:-1:round(lenn/10)]

        y_d_all=np.array(y_s)

        y_d = y_d_all[:, 0:-1:round(lenn/10)]


        theta_all=np.array(theta_s)

        theta = theta_all[:, 0:-1:round(lenn /10)]

        deltaf_all=np.array(deltaf_s)

        deltaf = deltaf_all[:, 0:-1:round(lenn /10)]

        deltar_all=np.array(deltar_s)

        deltar = deltar_all[:, 0:-1:round(lenn /10)]
        ...
        anim = animation.FuncAnimation(fig, animate, init_func=init,
                                   frames=len(x_d[0,:]), interval=20, blit=False)
    ...
    #print(ts[i],Vr_s[0][i],Vf_s[0][i])
    return waypoints

#wp=gui_rock([1,2,0],[3,4,pi/2],600,21,60)
#print(np.shape(wp),'wpppspspspspsppppppppppppppp11111111111111111')
