...
points = [[coord[0][0], coord[0][1], ang[0]], [coord[1][0], coord[1][1], ang[1]], [coord[2][0], coord[2][1], ang[2]],
          [coord[3][0], coord[3][1], ang[3]]]
...
for k in range(M * (N - 1)):
    ...
    Vr1_s[k] = sqrt((Vx1_s[k] ** 2 + Vy1_s[k] ** 2 + (omega1_s[k] * Lr) ** 2 + (
                2 * omega1_s[k] * Lr * (Vx1_s[k] * sin(theta1_s[k]) - Vy1_s[k] * cos(theta1_s[k])))))
    Vr2_s[k] = sqrt((Vx2_s[k] ** 2 + Vy2_s[k] ** 2 + (omega2_s[k] * Lr) ** 2 + (
                2 * omega2_s[k] * Lr * (Vx2_s[k] * sin(theta2_s[k]) - Vy2_s[k] * cos(theta2_s[k])))))
    Vr3_s[k] = sqrt((Vx3_s[k] ** 2 + Vy3_s[k] ** 2 + (omega3_s[k] * Lr) ** 2 + (
                2 * omega3_s[k] * Lr * (Vx3_s[k] * sin(theta3_s[k]) - Vy3_s[k] * cos(theta3_s[k])))))

    Vf1_s[k] = sqrt((Vx1_s[k] ** 2 + Vy1_s[k] ** 2 + (omega1_s[k] * Lf) ** 2 + (
                2 * omega1_s[k] * Lf * (Vy1_s[k] * cos(theta1_s[k]) - Vx1_s[k] * sin(theta1_s[k])))))
    Vf2_s[k] = sqrt((Vx2_s[k] ** 2 + Vy2_s[k] ** 2 + (omega2_s[k] * Lf) ** 2 + (
                2 * omega2_s[k] * Lf * (Vy2_s[k] * cos(theta2_s[k]) - Vx2_s[k] * sin(theta2_s[k])))))
    Vf3_s[k] = sqrt((Vx3_s[k] ** 2 + Vy3_s[k] ** 2 + (omega3_s[k] * Lf) ** 2 + (
                2 * omega3_s[k] * Lf * (Vy3_s[k] * cos(theta3_s[k]) - Vx3_s[k] * sin(theta3_s[k])))))
...
print(points,'pointsssssssssssssssssssssssssssssssssssssssssssssssssssssss')
...
def update(x, y, theta, Vf, Vr, deltaf, deltar, dt):
    ...
    x = x + (vx * dt) - dt*p_n[0] -m_n[0]
    y = y + (vy * dt) - dt*p_n[1] - m_n[1]
    theta = theta + (omega * dt) + dt*p_n[2] - m_n[2]
...
def find_closest_point(pose, reference_path, start_index):
    ...
    xlist = reference_path[0,start_index:] - pose[0]
    ylist = reference_path[1,start_index:] - pose[1]
...
ref_path=np.array([[i[0] for i in waypoints],[i[1] for i in waypoints]])
waypoints=np.array(waypoints)
print(np.shape(waypoints))
print(waypoints,'waypppppppppppppppppppppppp')
...
for ind_last in range(len(waypoints)):
    ...
    inp = [Vx_i[ind_last], Vy_i[ind_last], omega_i[ind_last]]

    if ind_last != len(points) - 1:
        dt = diff_t[ind_last]
    else:
    ...
    print(waypoints[ind_last,0],'wplllllllllllllllllllllllllllll')
    plt.plot(waypoints[ind_last, 0], waypoints[ind_last, 1], "or", label="course")
