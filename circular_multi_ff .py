...
def create_ocp_stage(ocp):
    ...
    stage = ocp.stage(t0=FreeTime(0), T=100)
    x = stage.state()
    y = stage.state()
    theta=stage.state()
    ...
    stage.method(MultipleShooting(N=20, M=5, intg='rk'))
...
def controller(ref,inp ,x, y, theta):
    # Proportional control inputs are calculated
    Lf = 0.5
    Lr = 0.5
    vmax = 0.0123
...
for j in range(0,len(points)):
    ...
    while(abs(p_err[0])>=0.01 or abs(p_err[1])>=0.01 or abs(p_err[2])>=0.01):
        ...
        [p_err,Vf,Vr,deltaf,deltar] = controller(ref,inp, x, y, theta)

        print( x, 'xxxxxxx11111111111111')
...
print(len(deltaf),'deltaffffffff')
...
anim = animation.FuncAnimation(fig, animate, init_func=init,
                               frames=len(x_d)-3, interval=200, blit=False)
