...
if __name__ == '__main__':
    ...
    # init_loc=[[7.5,7.4,3.111],[7.3,7.2,3.222],[7.1,7,3.333]]
    ...
    mainframe = Frame(root, width=frame_width, height=frame_height)
    mainframe.grid()
    ...
    Slider1 = Scale(mainframe, from_=5, to=20, length=frame_width, tickinterval=1, variable=gui_variable1,
                    orient=HORIZONTAL, resolution=0.5)
    Slider1.set(15)
    Slider1.grid(row=1, column=2)
    Slider2 = Scale(mainframe, from_=5, to=20, length=frame_width, tickinterval=1, variable=gui_variable2,
                    orient=HORIZONTAL, resolution=0.5)
    Slider2.set(15)
    Slider2.grid(row=3, column=2)
    Slider3 = Scale(mainframe, from_=1, to=10, length=frame_width, tickinterval=1, variable=gui_variable3,
                    orient=HORIZONTAL)
    Slider3.set(3)
    Slider3.grid(row=5, column=2)

    # Executing button that closes GUI and continues with program
    Button(mainframe, text='design choreography', command=creating).grid(row=6, columnspan=3)

    # canvas dimensions

    cw = round(frame_width / 1.1)
    ch = round(frame_height / 1.1)
    ...
    def create_rect(cw, ch, x, y, angle, canvas, tag, rob, outline):  # center coordinates, radius
        ...
        return canvas.create_polygon(x1, y1, x2, y2, x3, y3, x4, y4, tags=tag, fill=fill, outline=out, dash=(3, 3))
    ...
    t_g = [['rect11', 'rect12', 'rect13', 'rect14'], ['rect21', 'rect22', 'rect23', 'rect24'],
           ['rect31', 'rect32', 'rect33', 'rect34'], ['rect41', 'rect42', 'rect43', 'rect44'],
           ['rect51', 'rect52', 'rect53', 'rect54'], ['rect61', 'rect62', 'rect63', 'rect64'],
           ['rect71', 'rect72', 'rect73', 'rect74'], ['rect81', 'rect82', 'rect83', 'rect84'],
           ['rect91', 'rect92', 'rect93', 'rect94'], ['rect101', 'rect102', 'rect103', 'rect104']]
    ...
    def myfunction(event):
        ...
        if root.counter < sum(Nw):
            ...
            create_rect(cw, ch, x, y, 0, canvas, t_g[m][lp_count - 1], fil, out)
    ...
    def get_slider(event, l, k):
        ...
        create_rect(cw, ch, coord[l][k][-2], coord[l][k][-1], ang_l[l][k][-1], canvas, t_g[l][k], fil, out)
        if k == (sum(Nw) - 1):
    ...
    for j in range(Nr):
        for i in range(Nw[j]):
            ...
            Slider[j][i] = Scale(master=frame_a, from_=0, to=360, length=round(frame_width / 6),
                                 width=round(frame_height / 100), tickinterval=60,
                                 variable=gui_variable[j][i], orient=HORIZONTAL, command=partial(get_slider, l=j, k=i))
            Slider[j][i].set(0)
            Slider[j][i].grid(row=i, column=j)
    ...
    canvas.create_line(cw / 2 - 20, cw / 2, cw / 2 + 20, cw / 2, width=3, fill='green')
    canvas.create_line(ch / 2, ch / 2 - 20, ch / 2, ch / 2 + 20, width=3, fill='green')

    ############put grid lines#####################

    # Creates all vertical lines at intevals of 100
    for i in range(0, cw, 25):
        canvas.create_line([(i, 0), (i, ch)], tag='grid_line')

    # Creates all horizontal lines at intevals of 100
    for i in range(0, ch, 25):
        canvas.create_line([(0, i), (cw, i)], tag='grid_line')

    canvas.bind("<Button-1>", myfunction)
    canvas.pack()

    frame_d = Frame()
    lb2 = Label(master=frame_d,
                text="select points by clicking on grey canvas and select angles using sliders and close screen ",
                fg="green", bg="white", width=100,
                height=3)
    lb2.pack()
...
p1 = []
p2 = []
print(len(coord_w), Nr)
for i in range(Nr):
    coord_i = coord_w[i][0]
    p1.append([coord_i[0], coord_i[1], ang_w[i][0]])

for i in range(Nr):
    coord_i = coord_w[i][1]
    p2.append([coord_i[0], coord_i[1], ang_w[i][1]])
