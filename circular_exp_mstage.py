...
Fin_con = [[0, -1.5, pi],[-1.5, -1.5, 0],[1.5,-1.5,pi/2],[1.5,0,0]]
...
def create_stage(ocp):
    ...
    for i in range(robotN):
        ...
        omega[i] = stage.control(order=1)
        Vx[i] = stage.control(order=1)
        Vy[i] = stage.control(order=1)
        ...
        stage.set_der(x[i], Vx[i])
        stage.set_der(y[i], Vy[i])
        stage.set_der(theta[i], omega[i])

        # Initial constraints
        stage.subject_to(stage.at_t0(x[i]) == Init_con[i][0])
        stage.subject_to(stage.at_t0(y[i]) == Init_con[i][1])
        stage.subject_to(stage.at_t0(theta[i]) == Init_con[i][2])

        # Final constraint
        stage.subject_to(stage.at_tf(x[i]) == Fin_con[i][0])
        stage.subject_to(stage.at_tf(y[i]) == Fin_con[i][1])
        stage.subject_to(stage.at_tf(theta[i]) == Fin_con[i][2])
        ...
        stage.subject_to(0 <= (Vrs[i] <= vmax**2))
        stage.subject_to(0<= (Vfs[i] <= vmax**2))

        stage.subject_to(-7.5 <= (x[i] <= 7.5))
        stage.subject_to(-7.5 <= (y[i] <= 7.5))
    ...
    stage.add_objective(10 * stage.T)
...
points=np.array([[[[0 for i in range(3)] for j in range(2)]for k in range(3)]for l in range(3)])

M=2
N=21

# Shoot up the ball
stage1, x1, y1,theta1,Vx1,Vy1,omega1 = create_stage(ocp)
ocp.subject_to(stage1.t0 == 0)  # Stage starts at time 0
# Initial constraints
stage1.subject_to(stage1.at_t0(x1) == points[:,0,0,0])
stage1.subject_to(stage1.at_t0(y1) == points[:,0,0,1])
stage1.subject_to(stage1.at_t0(theta1) == points[:,0,0,2])

# Final constraint
stage1.subject_to(stage1.at_tf(x1) == points[:,0,1,0])
stage1.subject_to(stage1.at_tf(y1) == points[:,0,1,1])
stage1.subject_to(stage1.at_tf(theta1) == points[:,0,1,2])

stage1.set_initial(x1, np.linspace(points[:,0,0,0],points[:,0,1,0],N))
...
# After bounce 1
stage2, x2, y2,theta2,Vx2,Vy2,omega2 = create_stage(ocp)
